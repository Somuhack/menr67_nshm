...
const AddProduct = async (req, res) => {
  try {
    ...
    const pimagefile = req.file.filename
    if (!pname || !pprice || !pdes || !brandName || !review || !pimagefile) {
      return res
        .status(404)
        .json({ msg: "All Field Required!!", success: false });
    }
    const product = new Product({ pname, pprice, pdes, brandName, review ,pimage:pimagefile});
